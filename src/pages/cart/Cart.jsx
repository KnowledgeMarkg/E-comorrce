...
function Cart() {
  ...
  const buyNow = async () => {
    ...
    const options = {
      key: "rzp_test_Evb4xnK7ZlCmda",
      key_secret: "Gf5DbQGFRFKJYjkRSDkl3VMX",
  ...
  return (
    <Layout >
      <div className="flex flex-col min-h-screen bg-gray-100 pt-5"
        style={{
        ...
        {cartItems.length> 0 ? (
          <div className="mx-auto max-w-5xl px-6 md:flex md:space-x-6 xl:px-0 overflow-x-auto">
            ...
            {cartItems.map((item) => {
              ...
              return (
                <div key={item.id} className="justify-between mb-6 rounded-lg border  drop-shadow-xl bg-white p-6  sm:flex  sm:justify-start" style={{ backgroundColor: mode === 'dark' ? 'rgb(32 33 34)' : '', color: mode === 'dark' ? 'white' : '', }}>
                  <img src={imageUrl} alt="" className="w-full rounded-lg sm:w-40" />
          ...
          <p className="text-center font-extrabold text-3xl flex h-screen justify-center items-center " style={{ color: mode === 'dark' ? 'white' : '' }}>Your cart is empty</p>
        )}
      </div>
    </Layout>
  )
